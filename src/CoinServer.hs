...
serve = do 
    ...
    serverWith defaultConfig { srvLog = stdLogger, srvPort = 8888 }
     $ \_ url request -> do
      atomically $ readTVar shared >>= writeTVar shared . (+1)   
      requests <- atomically $ readTVar shared
        ...
        GET ->
          do
            ...
            case mb_txt of
               ...
               Left (e :: SomeException) -> return $ sendHTML NotFound $
                   thehtml $ concatHtml
                     [ thead noHtml
                     , body $ concatHtml
                        [ toHtml "In total, there have been "
                        , toHtml $ show requests
                        , toHtml " requests so far"
                        ]
                     ]
